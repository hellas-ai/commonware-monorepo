...
pub(crate) trait FromSyncTestable: qmdb::sync::Database {
    ...
    fn pinned_nodes_from_map(&self, pos: Position) -> impl std::future::Future<Output = Vec<Self::Digest>> + Send;
}
    ...
    JournalOf<H>: Contiguous,
{
    ...
    executor.start(|mut context| async move {
        ...
        let pinned_nodes =
            sync_db.pinned_nodes_from_map(Position::try_from(sync_db_original_size).unwrap()).await;
