    ...
    E: Elector<S>,
{
    ...
    fn should_nullify_by_contradiction(&self, view: View) -> bool {
        ...
        let Phase::Voted { .. } = *state.phase() else {
            return false;
        };
        ...
        let voted_proposal = self
            .voted_proposal(view)
            .expect("voted_proposal must exist when phase is Voted");
        let mut signers = BTreeSet::new();
        for notarize in tracker.iter_notarizes() {
            if notarize.proposal != voted_proposal {
    ...
    fn nullify_by_certificate_contradiction(
        ...
        let voted_proposal = self
            .voted_proposal(view)
            .expect("voted_proposal must exist when phase is Voted");
        if voted_proposal == *certificate_proposal {
            return None;
        }
...
mod tests {
    ...
    /// Validates that when L-quorum is reached for proposal B while proposal A
    /// already has an M-notarization at the same view, the M-notarization for B
    /// is still broadcast.
    ///
    /// This works because M-quorum (3 votes) is always reached before L-quorum
    /// (5 votes) during sequential vote processing, causing the M-not for B to
    /// be broadcast at M-quorum time. The `any_m_notarization(view)` check in
    /// the L-quorum path then correctly skips a redundant M-not broadcast.
    ///
    /// Note: The `any_m_notarization` check is imprecise (checks for ANY M-not,
    /// not the finalized proposal's M-not). This is safe only because sequential
    /// vote processing guarantees M-quorum is always hit before L-quorum. If the
    /// processing order ever changes, this test would catch the regression.
    #[test]
    fn l_quorum_broadcasts_m_notarization_for_finalized_proposal() {
        let (mut state, schemes, mut rng) = setup_state();
        let genesis = Sha256Digest::from([0u8; 32]);

        // Create two proposals for view 1 (Byzantine leader equivocation)
        let proposal_a = Proposal::new(
            Round::new(Epoch::new(1), View::new(1)),
            View::zero(),
            genesis,
        ...
        // Nullify first so we don't vote for A via vote2 rule when M-not arrives
        let result = state.handle_timeout();
        assert!(result.nullify.is_some(), "Should nullify on timeout");

        // Receive M-notarization certificate for proposal A (from signers 1, 2, 3).
        // We don't vote for A because we already nullified.
        let notarizes_a: Vec<_> = schemes
            .iter()
            .skip(1)
            .take(3)
        ...
        let m_not_a =
            MNotarization::from_notarizes(&schemes[1], notarizes_a.iter(), &Sequential).unwrap();
        let actions = state.receive_certificate(Certificate::MNotarization(m_not_a));
        assert!(
            actions
                .iter()
                .any(|a| matches!(a, Action::Advanced(v) if *v == View::new(2))),
        ...
        // Now collect L-quorum (5 = n-f) individual votes for proposal B.
        // Use signers 1..=5 (not signer 0 = us, who nullified).
        // The M-not certificate for A doesn't consume tracker slots, so all
        // 5 votes insert cleanly.
        let mut saw_finalization = false;
        let mut saw_m_not_broadcast_for_b = false;
        for scheme in schemes.iter().skip(1) {
            let vote = Notarize::sign(scheme, proposal_b.clone()).unwrap();
            let actions = state.receive_notarize(&mut rng, vote, &Sequential);
            for action in &actions {
                if matches!(action, Action::Finalized(_)) {
                    saw_finalization = true;
                }
                if matches!(
                    action,
                    Action::BroadcastCertificate(Certificate::MNotarization(m))
                    if m.proposal == proposal_b
                ) {
                    saw_m_not_broadcast_for_b = true;
                }
            }
        }

        assert!(saw_finalization, "Should have finalized proposal B");
        assert!(
            saw_m_not_broadcast_for_b,
            "Must broadcast M-notarization for proposal B even though proposal A \
             already has an M-notarization at this view"
        );
    }
}
