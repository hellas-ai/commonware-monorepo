...
commonware_macros::stability_scope!(ALPHA {
    ...
    pub trait Scheme: Debug + Clone + Send + Sync + 'static {
        ...
        type StrongShard: Clone + Eq + Codec<Cfg = CodecConfig> + Send + Sync + 'static;
        ...
        /// the data.
        type WeakShard: Clone + Eq + Codec<Cfg = CodecConfig> + Send + Sync + 'static;
    ...
    fn general_test<S: Scheme>(
        ...
        let ((_, _, checking_data, my_checked_shard, _), other_packets) = {
            let mut out = shards
                ...
                .enumerate()
                .filter_map(|(i, shard)| {
                    let shard = S::StrongShard::read_cfg(&mut shard.encode(), &read_cfg).unwrap();
                    let i = i as u16;
                    let pos_of_i = indices.iter().position(|&x| x == i)?;
                    let (x0, x1, x2) = S::weaken(&config, &commitment, i, shard).unwrap();
        ...
        let checked_shards = {
            let mut others = other_packets
                .into_iter()
                .map(|(_, i, _, _, weak_shard)| {
                    let weak_shard =
                        S::WeakShard::read_cfg(&mut weak_shard.encode(), &read_cfg).unwrap();
                    S::check(&config, &commitment, &checking_data, i, weak_shard).unwrap()
                })
                .collect::<Vec<_>>();
